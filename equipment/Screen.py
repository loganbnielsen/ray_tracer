...
class Screen:
    ...
    def __init__(self, height, width):
        ...
        # generate pixelgrid
        self.grid = Grid(self.min_x, self.max_x, self.min_y, self.max_y, self.height, self.width)


class Grid:
    def __init__(self, min_x, max_x, min_y, max_y, height, width):
        self.coords = self._gen_coords(min_x, max_x, min_y, max_y, height, width)
        self.colors = np.zeros(self.coords.shape) # RGB channels
        # TODO init colors at background color
    
    def _gen_coords(self, min_x, max_x, min_y, max_y, height, width):
        """
        ...
        def _axis_array(min_val, max_val, n):
            ...
            """
            line = np.linspace(min_val, max_val, n, endpoint=False) # (bottom/left) side of each pixel starting from (bottom/left)
            width = line[1] - line[0]
